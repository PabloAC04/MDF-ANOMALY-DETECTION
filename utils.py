...
def hampel_cpu(x: np.ndarray, window: int = 25, sigma: float = 5.0,
               repair: str = "median", shrink_lambda: float = 0.7):
    """
    Hampel filter vectorizado en CPU (numpy + scipy).
    x: array 1D (np.ndarray).
    """
    k = 1.4826
    ...
    # (podrías implementar "interp" con np.interp si lo necesitas)
